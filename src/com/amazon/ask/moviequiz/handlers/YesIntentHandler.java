...
public class YesIntentHandler implements IntentRequestHandler {
    ...
    public boolean canHandle(HandlerInput input, IntentRequest intentRequest) {
        return (input.matches(intentName("AMAZON.YesIntent")
                .or(intentName("NextMovieIntent"))
                .or(intentName("AMAZON.StartOverIntent"))));
    }

    @Override
    public Optional<Response> handle(HandlerInput input, IntentRequest intentRequest) {
        final String speechText = intentRequest.getIntent().getName().equals("NextMovieIntent") ?
                Constants.NEXT_MOVIE_SPEECH : Constants.PICK_MOVIE_SPEECH;
        ...
        dataModel.put("aplDocument", getJsonString("/whichmovie.json", YesIntentHandler.class));
        dataModel.put("aplData", "{}");
